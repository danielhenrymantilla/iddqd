...
impl BiHashMapMachine {
    ...
    fn entry_insert_overwrite(&mut self, tc: TestCase) {
        let item = tc.draw(test_item());
        let map_res = match self.map.entry(item.key1(), item.key2()) {
        ...
        let occupied = self.naive.get1(item.key1).is_some()
            || self.naive.get2(item.key2).is_some();
    ...
    fn entry_remove(&mut self, tc: TestCase) {
        ...
        let map_res =
            match self.map.entry(TestKey1::new(&key1), TestKey2::new(key2)) {
        ...
        let mut naive_res = self.naive.entry_remove12(key1, key2);
        naive_res.sort();
    ...
    fn get_unique(&mut self, tc: TestCase) {
        let (key1, key2) = draw_lookup_keys12(&tc, &self.naive);
        let map_res =
            self.map.get_unique(&TestKey1::new(&key1), &TestKey2::new(key2));
        let naive_res = self.naive.get_unique12(key1, key2);
